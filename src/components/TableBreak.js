...
export default function TableBreak() {
  ...
  const goBreakDisabled =
    user?.data?.user?.state === "WAITING" ||
    user?.data?.user?.state === null ||
    user?.data?.user?.state === "IN_BREAK";
  const inBreak = () => {};
  const isAdminOrYourMessage =
    user?.data?.user?.id === user?.data?.messageId?.userId || isAdmin;
  if (isLogin) {
    return (
      <>
        {!isAdmin && (
          <div style={{ position: "absolute", top: 15, left: 15 }}>
            ...
            <button onClick={sendMessage} disabled={isMessage}>
              Send Message
            </button>
          </div>
        )}
        {!isAdmin && (
          <div style={{ position: "absolute", top: "10%", right: "15%" }}>
            {user?.data?.user?.breakTime <= 0 ? (
              <button
                disabled={extraIsSuccess}
                onClick={exhort}
                style={{
                  borderWidth: 0.1,
                  backgroundColor: extraIsSuccess ? "#c7c7c2" : "green",
                  borderRadius: 0,
                  color: "white",
                  marginRight: 15,
                }}
              ...
              <button
                disabled={exhortDisabled}
                onClick={exhort}
                style={{
                  borderWidth: 0.1,
                  backgroundColor: exhortDisabled ? "#c7c7c2" : "green",
                  borderRadius: 0,
                  color: "white",
                  marginRight: 15,
                }}
            ...
            <button
              disabled={goBreakDisabled}
              onClick={confirmGoBreak}
              style={{
                borderWidth: 0.1,
                backgroundColor: goBreakDisabled ? "#c7c7c2" : "#F7A91C",
                borderRadius: 0,
                color: "white",
                marginRight: 15,
              }}
            ...
            <button
              disabled={cancelBreakDisabled}
              onClick={confirmCancelBreak}
              style={{
                borderWidth: 0.1,
                backgroundColor: cancelBreakDisabled ? "#c7c7c2" : "#F7A91C",
                borderRadius: 0,
                color: "white",
                marginRight: 15,
              }}
            ...
            <button
              disabled={cancelBreakDis}
              onClick={cancel}
              style={{
                borderWidth: 0.1,
                backgroundColor: cancelBreakDis ? "#c7c7c2" : "red",
                borderRadius: 0,
                color: "white",
              }}
            >
          ...
          columns={[
            {
              ...
              render: ({ lastName, name, jobType }) => (
                <Text
                  weight={700}
                  style={{ padding: 10 }}
                  color={jobType === "CALL" ? "blue" : "#955c23"}
                >
            ...
            },
            {
              accessor: "message",
              render: ({ message }) => (
                <Text weight={700}>{isAdmin && message}</Text>
              ),
            },
            {
              accessor: "applyDate",
              render: ({ applyDate }) => <Text weight={700}>{applyDate}</Text>,
            },
            {
              accessor: "breakDate",
              render: ({ inBreakDate }) => (
                <Text weight={700}>{inBreakDate}</Text>
              ),
            },
          ]}
